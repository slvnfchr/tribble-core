...
PluginStream.prototype._transform = function transform(file, encoding, done) { // eslint-disable-line no-underscore-dangle, max-len
	this.plugin.process(file).then((processedFile) => {
...
function Plugin(input, output, process) {
	...
	this.input = input;
	this.output = output;
	this.process = file => new Promise(done => process(file, done));
}
